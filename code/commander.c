...
void execute_command(char **args, char *input_file, char *output_file, int append_output)
{
    ...
    if (pid == 0) // Child process
    {
        ...
        // TODO: Implement redirection for custom commands
        if (!execute_custom_command(args, 4096, NULL, NULL, NULL, NULL))
        {
...
void execute_piped_commands(char *command, const char *home_dir, char **prev_dir, char **last_command, char *aliases[4096])
{
    ...
    *(end + 1) = '\0';
    ...
    for (int i = 0; i < pipe_count; i++)
    {
        ...
        // TODO: Do this foreground and background handling just like the commander function
        pid_t pid = fork();
        if (pid == 0) // Child process
        {
            // Set up pipes
    ...
    for (int j = 0; j < num_commands; j++)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (!execute_custom_command(args, num_args, home_dir, prev_dir, last_command, aliases))
                {
